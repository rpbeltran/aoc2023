...
DIM SHARED hindex (width, width, 4, 11) as LONG
DIM SHARED hstate as HeapState
DIM SHARED hdata ((width * width * 4 * 11) + 1) as HeapEntry
...
DIM dist (width, width, 4, 11) as LONG
DIM explored (width, width, 4, 11) as Integer
...
SUB GetNeighborhood (entry as HeapEntry, width)
    ...
    FOR TURN=3 TO 5
        ...
        IF NOT (neighbor.x < 0 OR neighbor.x >= width OR neighbor.y < 0 OR neighbor.y >= width OR neighbor.dtime > 10) THEN
            IF entry.dtime > 3 OR neighbor.direction = entry.direction THEN
